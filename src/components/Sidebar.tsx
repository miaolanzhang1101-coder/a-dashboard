...
export function Sidebar({ period, onCollapse, className = "" }: { period: PeriodKey; onCollapse?: () => void; className?: string }) {
  ...
  const subItems: SubItem[] = [
    { label: "Marketing", href: `/source${q}#marketing`, section: "marketing" },
    { label: "Insights", href: `/source${q}#insights`, section: "insights" },
  ];
  ...
  useEffect(() => {
    const ids = ["bookings", ...subItems.map((s) => s.section)];
  ...
  return (
    <aside className={`flex w-60 shrink-0 flex-col bg-surface ring-1 ring-hairline ${className}`}>
      <div className="flex h-14 items-center justify-between gap-2 px-4">
        ...
        {onCollapse && (
          <button
            ...
            className="grid size-8 place-items-center rounded-md text-ink-3 transition-colors duration-150 hover:bg-canvas hover:text-ink"
          >
      ...
      {/* property switcher (opens a menu) */}
      <div ref={menuRef} className="relative px-3 pb-3 pt-1">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-lg bg-canvas px-3 py-2 text-left shadow-ambient ring-1 ring-hairline transition-all duration-200 hover:shadow-ambient-md active:scale-[0.99]"
        >
          ...
          <svg className={`size-3.5 shrink-0 text-ink-3 transition-transform duration-150 ${menuOpen ? "rotate-180" : ""}`} viewBox="0 0 16 16" fill="none">
            <path d="M5 6.5 8 9.5l3-3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
        {menuOpen && (
        ...
        <ul className="space-y-1">
          <li>
            <Link
              ...
              className={`group flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all duration-150 ${overviewActive ? "bg-canvas font-medium text-ink shadow-ambient ring-1 ring-hairline" : "text-ink-2 hover:bg-canvas/70 hover:text-ink"}`}
            >
          ...
          <li>
            <div className={`flex items-center rounded-lg pr-1 transition-all duration-150 ${bookingsActive ? "bg-canvas shadow-ambient ring-1 ring-hairline" : "hover:bg-canvas/70"}`}>
              <Link
                href={`/source${q}#bookings`}
                aria-current={bookingsActive ? "page" : undefined}
                className={`group flex min-w-0 flex-1 items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors duration-150 ${bookingsActive ? "font-medium text-ink" : "text-ink-2 hover:text-ink"}`}
              >
                <svg className={`size-4 shrink-0 ${bookingsActive ? "text-accent" : "text-ink-3 group-hover:text-ink-2"}`} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round">{ICONS.bookings}</svg>
                <span className="truncate">Bookings</span>
              </Link>
              <button
                type="button"
                onClick={() => setExpanded((v) => !v)}
                aria-label={expanded ? "Collapse Bookings" : "Expand Bookings"}
                aria-expanded={expanded}
                className="grid size-7 shrink-0 place-items-center rounded-md text-ink-3 transition-colors duration-150 hover:text-ink"
              >
            ...
            {expanded && (
              <ul className="mt-1 space-y-0.5 pl-5">
                {subItems.map((s) => {
                  const active = bookingsActive && visible === s.section;
                  return (
                    <li key={s.section}>
                      <Link
                        href={s.href}
